...
export const currentNetTarrifByDate = ({
	...
	const mostRecentTariff = relevantNetTariffs.reduce((previousTariff, currentTariff) => {
		const previousTariffDate = DateTime.fromISO(previousTariff.fromDate, { zone: 'utc' });
		const currentTariffDate = DateTime.fromISO(currentTariff.fromDate, { zone: 'utc' });
		return previousTariffDate > currentTariffDate ? previousTariff : currentTariff;
	});
