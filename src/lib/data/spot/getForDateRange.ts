 ...
 */
export const getForDateRange = async (
  params: Params,
): Promise<InternalResponse<SpotData[]>> => {
  ...
  if (!dateRangeIsValid) {
    return returnError(
      400,
      '🤦‍♂️ The "to" date must be later than the "from" date',
    );
  }
  ...
  if (!dateRangeIsWithinDBLimits) {
    return returnError(
      400,
      `😞 Period is too long. Requested: ${dataHourDiff}. Max: ${LIMIT}.`,
    );
  }
  ...
  if (gotExpectedData) {
    const requestedDatapoints = filterDataToInterval({
      from,
      to,
      data: dbSpotResponse.data,
    });
  ...
  const newDataPoints = filterSpotData({
    newEntries: apiSpotData,
    existingEntries: dbSpotData,
  });
  ...
  if (saveSpotResponse.success === false) {
    ...
    console.log(
      "🚫",
      `[${saveSpotResponse.error.code}] ${saveSpotResponse.error.message}`,
    );
  }
